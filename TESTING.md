...
**A note on the SEO scores below:** the Bag, Checkout, Profile, Admin Dashboard, and admin form screenshots still show 66-69 on SEO rather than 100. That was caused by `robots.txt` disallowing `/bag/`, `/checkout/`, `/profiles/`, and the `/dishes/add/`, `/dishes/edit/`, `/dishes/delete/`, and `/dishes/category/` paths, which made Lighthouse's `is-crawlable` audit fail on each of them - `/admin/` and `/accounts/` remain disallowed (the Django admin panel and login/auth flows should never be indexed), but the rest of that list has since been removed, so these pages are crawlable now. The screenshots below predate that change and should be regenerated to confirm 100.
