"""
Models for Dish, DishPortion, DishImage, and Category.
(managers, querysets, and image compression logic)
"""
...
class Dish(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.available_from and self.available_until:
            if self.available_from > self.available_until:
                raise ValueError(
                    "available_from must be before or equal to available_until"
                )
    ...
    def _compress_image_field(
        self,
        field_name,
        max_size=(1200, 1200),
        quality=75,
        card_max_size=(800, 800),
        card_quality=60,
    ):
        """
...
class DishPortion(models.Model):
    ...
    size = models.CharField(
        max_length=50,
        help_text="Portion size (e.g., Small, Medium, Large)"
    )
